...
# writer = SummaryWriter(comment='logs/coco_resnet_50_HSAM_TA_obj')
...
def evaluate_hoi(dataset_file, model, postprocessors, data_loader, subject_category_id, device, out_dir, epoch, args):
    ...
    if dataset_file == 'hico':
        ...
        #     writer.add_scalar('mAP_def/mAP_def_rare', stats['mAP_def_rare'], epoch)
        #     writer.add_scalar('mAP_def/mAP_def_non_rare',stats['mAP_def_non_rare'] , epoch)
        #     writer.add_scalar('mAP_ko/mAP_ko', stats_ko['mAP_ko'], epoch)
        #     writer.add_scalar('mAP_ko/mAP_ko_rare',stats_ko['mAP_ko_rare'] , epoch)
        #     writer.add_scalar('mAP_ko/mAP_ko_non_rare', stats_ko['mAP_ko_non_rare'], epoch)
        ...
        # if utils.is_main_process():              
        #     writer.add_scalar('mAP/mAP_all',stats['mAP_all'] , epoch)
        #     writer.add_scalar('mAP/mAP_thesis', stats['mAP_thesis'], epoch)

    elif dataset_file == 'hake':
        evaluator = HAKEEvaluator(args.hake_split_test, preds, gts, args.hake_path, out_dir, epoch, use_nms=args.use_nms, nms_thresh=args.nms_thresh)
        rank = utils.get_rank()
        stats = evaluator.evaluation_default()
        # if utils.is_main_process():              
        #     writer.add_scalar('mAP_def/mAP_def',stats['mAP_def'] , epoch)
